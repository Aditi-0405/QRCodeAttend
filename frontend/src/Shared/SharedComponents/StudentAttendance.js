...
const StudentAttendance = () => {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      try {
        const response = await fetch(`https://${process.env.REACT_APP_BACKEND_BASE_URL}/api/admin/getStudentAttendance/${studentId}`, {
          method: 'GET',
          headers: {
  ...
  const handleToggleStatus = async (record) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://${process.env.REACT_APP_BACKEND_BASE_URL}/api/admin/updateAttendance`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
